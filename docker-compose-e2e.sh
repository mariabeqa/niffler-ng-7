...
#Устанавливаем переменную окружения BROWSER=firefox, если получен аргумент "firefox"
# и скачиваем образ selenoid/vnc_firefox:125.0
if [ "$1" = "firefox" ]; then
...
#Проходим по списку образов из docker-compose.yml. Если хотя бы один образ отсутствует, то собираем образы через jibDockerBuild
